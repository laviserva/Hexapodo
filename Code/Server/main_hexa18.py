...
def ejecutar_estado_con_reintento(estado_func, *args, max_retries=5, retry_interval=2):
    intentos = 0
    while intentos < max_retries:
        try:
            return estado_func(*args)
        except Exception as e:
            print(f"[ERROR] Error al ejecutar el estado '{estado_func.__name__}': {e}")
            print(f"[REINTENTO] Intento {intentos + 1}/{max_retries} en {retry_interval} segundos...")
            time.sleep(retry_interval)
            intentos += 1
    print(f"[FALLIDO] No se pudo completar el estado '{estado_func.__name__}' después de {max_retries} intentos.")
    return None
...
if __name__ == '__main__':
    ...
    while BDI_Actions.ABORTAR not in intenciones:
        ...
        if BDI_Actions.BUSCAR_COMP in intenciones:
            ejecutar_estado_con_reintento(buscar_compañero, HexapodoComunicator, client_name, server_host)
            env.buddy_here = True
        if BDI_Actions.ESTABLECER_CONSENSO in intenciones:
            hexapodo_1, rutina, recibidos = ejecutar_estado_con_reintento(consenso, hexapodo_1, communicator)
        ...
        if BDI_Actions.EJECUTAR_SUB1 in intenciones or BDI_Actions.EJECUTAR_SUB2 in intenciones:
            ...
            ejecutar_estado_con_reintento(ejecutar_subrutinas, rutina, communicator)
            h1.completes += 1
