...
class BDIAgent:
    ...
    def percieve(self, environment):
        # Aquí se percibe el entorno y se generan beliefs con base en el entorno
    ...
    def generate_options(self):
        options = []
        if self.beliefs.get("acompañado") is True:
            options.append({"name": "establecer_consenso", "conflicts": []})
        else:
            options.append({"name": "buscar_compañero", "conflicts": []})

        if self.beliefs.get("puedo_terminar") is False:
            options.append({"name": "stop", "conflicts": ["ejecutar_subrutina1", "ejecutar_subrutina2",
                                                          "buscar_compañero", "establecer"]})

        if self.beliefs.get("compañero_enterado"):
            options.append("ejecutar_subrutina1")

        if self.beliefs.get("terminamos_parte1"):
            options.append("ejecutar_subrutina2")

        if self.beliefs.get("terminamos_parte2"):
            options.append("")
        return options
